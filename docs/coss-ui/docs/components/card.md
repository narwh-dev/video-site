...
For lists or data grids, render a [Table](/docs/coss-ui/docs/components/table.md) with `variant="card"` inside `CardFrame` so the grid matches the frame’s border and radius. See the CardFrame example in the Table docs. Add `@coss/table` if it is not already in the project.
