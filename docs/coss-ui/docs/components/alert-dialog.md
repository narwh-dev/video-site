...
Unlike [Dialog](/docs/coss-ui/docs/components/dialog.md), alert dialog has no `AlertDialogPanel`. Put title and description in `AlertDialogHeader`, actions in `AlertDialogFooter`, and use a plain element with `className` if you need extra body content between them.
